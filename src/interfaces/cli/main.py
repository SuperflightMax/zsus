...
def _print_llm_debug(result: Dict[str, Any]) -> None:
    """Print debug information from the interpreter if available."""

    debug = result.get("debug") or {}
    error = debug.get("error")
    raw = debug.get("raw_llm_response")

    if error:
        print(f"[llm-debug] error: {error}")
    if raw:
