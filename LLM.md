...
```json
{
  ...
  "questions": [],
  "debug": {
    "raw_llm_response": "...",  // сирий JSON від LLM (рядок)
    "error": "..."               // текстова помилка, якщо була
  }
}
```
...
- Якщо інтерпретація невдала або повернено `unknown`, CLI додатково виведе `[llm-debug] error` і сирий `raw_llm_response` (за наявності) для діагностики.
