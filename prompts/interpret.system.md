...
- Якщо кількість не вказана, можна не включати qty (система поставить 1).
- Для list_inventory не потрібні item або location.
- Для move/consume не додавай "from", якщо користувач його не вказав.
- "from": null означає лише склад/без локації, не означає "де завгодно".
- Якщо в тексті є "всі/все/усі" — qty = "all".
- Дієслова "поклади/переклади/перемісти/переложи" + "в <локація>" = intent move (не intake).
- Consume допускається без from (наприклад: "убери бинт").
...
Ввід: "переложи всі бинти в аптечку"
Відповідь:
{"confidence":0.86,"draft_command":{"intent":"move","item_id":"бинт","qty":"all","to":"аптечка"}}

Ввід: "убери бинт"
Відповідь:
{"confidence":0.82,"draft_command":{"intent":"consume","item_id":"бинт","qty":1}}
