...
MVP хранилище: SQLite (PostgreSQL поддерживается схемой и пригодится в будущем).
...
повторная приёмка увеличивает qty и не создаёт дубликаты (PRIMARY KEY по item_id+location_id)

При **move**
уменьшается stock для from_location
увеличивается stock для to_location
добавляется запись в movements
операция возможна только если предмет существует и остаток на `from` достаточен; при нарушении — ошибка и откат транзакции
