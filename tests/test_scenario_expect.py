...
def test_match_expect_ok_status_only():
    response = {"status": "ok", "data": {"foo": "bar"}}
...
@pytest.mark.parametrize(
    "response,expected",
    [
        ({"status": "ok", "data": {"total_qty": 8, "item_id": "abc"}}, {"total_qty": 8}),
        ({"status": "ok", "data": {"a": {"nested": 1}, "b": 2}}, {"a": {"nested": 1}}),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"status": "ok", "data": {"other": 1}}, {"total_qty": 8}),
        ({"status": "ok", "data": []}, {"total_qty": 8}),
        ({"status": "ok"}, {"total_qty": 8}),
    ],
)
