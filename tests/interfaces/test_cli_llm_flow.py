...
def test_handle_text_prints_llm_debug(monkeypatch, capsys):
    interpreter = _DummyInterpreter(
        {
            "intent": "unknown",
            "confidence": 0.0,
            "needs_confirmation": True,
            "human_summary": "Не вдалося інтерпретувати запит.",
            "command": None,
            "questions": ["?"],
            "debug": {"error": "boom", "raw_llm_response": '{"x":1}'},
        }
    )
    ...
    assert "[llm-debug] error: boom" in output
