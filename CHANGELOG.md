...
- Wired CLI plain-text input through the interpreter with optional confirmation and immediate execution when безпечна та підтверджена команда, documented LLM/CLI flow with new tests, and surfaced interpreter debug output (error + raw LLM response) in CLI when interpretation fails/returns unknown.
